...
class MotionFrames:
    ...
    def render(self, frame_id, include_marker=True):
        ...
        frame[y0:y1, x0:x1][mask] = self.colors[3]
    ...
    def marker(self, frame_id):
        block = self.definition["markerBlockPixels"]
        bits = self.definition["markerBits"]
        frame = np.empty((block, (bits + 2) * block, 4), dtype=np.uint8)
        self.draw_marker(frame, frame_id)
        return frame
...
class MotionCanvas(Gtk.Widget):
    ...
    def do_snapshot(self, snapshot):
        ...
        def rectangle(color, x, y, width, height):
            rect = Graphene.Rect().init(x * sx, y * sy, width * sx, height * sy)
...
class MotionApp(Gtk.Application):
    ...
    def on_tick(self, widget, frame_clock):
        ...
        self.tick_id += 1
        for counter, source_frame_id in list(self.pending_presentations.items()):
            ...
            self.log.write(json.dumps({
                ...
                "refreshIntervalMicroseconds": timings.get_refresh_interval(),
            }, separators=(",", ":")) + "\n")
            del self.pending_presentations[counter]
        refresh_us, predicted_us = frame_clock.get_refresh_info(frame_clock_us)
