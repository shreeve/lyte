...
# expect_version PREVIOUS FLOOR WANT: the successor at clock 1000.
expect_version() {
    local got
    got="$(LYTE_BUILD_EPOCH=1000 "$next_version" "$1" "$2")"
    [[ "$got" == "$3" ]] || fail "bundle version after $1 (floor $2) is $got; want $3"
}
expect_version 999 20 1000
expect_version 1000 20 1001
expect_version 1001 20 1002
expect_version 999 2000 2000

if LYTE_BUILD_EPOCH=invalid "$next_version" 0 0 >/dev/null 2>&1; then
    fail "bundle version accepted a non-numeric clock"
fi
if LYTE_BUILD_EPOCH=1000 "$next_version" previous 0 >/dev/null 2>&1; then
    fail "bundle version accepted a non-numeric predecessor"
fi
...
then
    fail "empty process table reported a Lyte process"
else
    [[ "$?" -eq 1 ]] || fail "empty process table did not report status 1"
fi
[[ "$(LYTE_PGREP="$fake_pgrep" LYTE_FAKE_PGREP_RESULT=app \
    lyte_app_processes)" == $'Lyte\t4242' ]] \
    || fail "the running app was not reported"
[[ "$(LYTE_PGREP="$fake_pgrep" LYTE_FAKE_PGREP_RESULT=helper \
    lyte_app_processes)" == $'lyte-helperd\t4343' ]] \
    || fail "the running helper was not reported"
if LYTE_PGREP="$fake_pgrep" LYTE_FAKE_PGREP_RESULT=error \
    lyte_app_processes >/dev/null
then
    fail "process inspection error reported quiescence"
else
    [[ "$?" -eq 2 ]] || fail "process inspection error did not report status 2"
fi
...
for result in app helper error; do
    ...
    then
        fail "make-app accepted process state: $result"
    fi
...
[[ -e "$lock_ready" ]] || fail "the lock holder never took the lock"
if (LYTE_APP_LOCK_FILE="$shared_lock"; lyte_acquire_app_artifact_lock) \
    2>"$test_root/lock.stderr"
then
    fail "artifact lock admitted a concurrent owner"
fi
...
then
    fail "make-app accepted an unreadable prior bundle version"
fi
[[ ! -e "$fake_swift_log" ]] || fail "make-app compiled over an unreadable bundle version"
...
then
    fail "make-app accepted a destination inside the live bundle"
fi
[[ ! -e "$fake_swift_log" ]] || fail "make-app compiled for a destination inside the live bundle"
grep -Fq 'isolated app destination cannot be inside' \
    "$test_root/nested.stderr"
[[ ! -e "$repo_root/.build/Lyte.app/Nested.app" ]] \
    || fail "make-app created a bundle inside the live bundle"
...
launch_order="$(tr '\n' ' ' < "$launch_log")"
[[ "$launch_order" == 'codesign codesign register open observe ' ]] \
    || fail "launch ran out of order: $launch_order"

if grep -Fq 'tccutil' "$launch_app"; then
    fail "launcher attempts an unsupported Local Network privacy reset"
fi
