...
run_gate() {
    ...
    # seeded ARQ simulation. A Wire suite uses @testable imports, which a
    # release build admits only with -enable-testing.
    echo "==> Wire tests, release, 25,000 ARQ trials"
    (cd "$gate_root/Wire" && LYTE_ARQ_TRIALS=25000 swift test -c release \
        -Xswiftc -enable-testing -Xswiftc -warnings-as-errors)
