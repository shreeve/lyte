// Everything the Noise layer can refuse. Hostile bytes throw one of these —
// never trap, and never carry key material in associated values.
...
public enum NoiseError: Error, Equatable, Sendable {
    ...
    case nonceExhausted
    /// Plaintext exceeds the 1112 B shard budget.
    ...
    case staleSequence
    /// This exact extended counter was already accepted once; replay
    /// protection admits each seq once.
    case replayedSequence
    /// The sender was asked to seal a (chan, seq) at or behind one it
    /// already sealed. A retransmit resends the sealed bytes, never
    /// re-seals, so a non-advancing counter is (key, nonce) reuse in the
    /// making and is refused.
