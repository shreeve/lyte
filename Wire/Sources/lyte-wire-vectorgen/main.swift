// Authoring tool for Wire/Vectors/envelope-v1.json. Run once, commit the
// output, and treat the committed file as frozen: a byte difference against
// it is a wire-contract break to investigate, never a prompt to regenerate.
// The circularity (vectors produced by the codec they test) is broken by
// the hand-computed anchor bytes in EnvelopeTests — the nominal vector's
// hex is verified against literals a human derived from the layout comment.
//
// Usage: swift run lyte-wire-vectorgen <output-path>
...
guard CommandLine.arguments.count == 2 else {
    FileHandle.standardError.write(
        Data("usage: lyte-wire-vectorgen <output-path>\n".utf8)
    )
    exit(64)
}
...
print("wrote \(file.vectors.count) vectors to \(CommandLine.arguments[1])")
