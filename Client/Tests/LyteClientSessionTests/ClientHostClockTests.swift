...
final class ClientHostClockTests: XCTestCase {
    ...
    func testOutOfOrderIngestMatchesInOrderExactly() throws {
        ...
        var inOrder = ClientHostClock()
        samples.forEach { inOrder.ingest($0) }

        var scrambled = ClientHostClock()
    ...
    func testTooFewSamplesDegradeToOffsetOnly() throws {
        var model = ClientHostClock(config: .init(minimumSamplesForSkew: 3))
    ...
    func testWindowIsBoundedByCount() throws {
        ...
        XCTAssertEqual(fit.windowSamples,
                       ClientHostClock.Config().maxWindowSamples)
