...
/// The client's model of the host clock, IO-free: the host↔client mapping
/// from the beacon book's (offset, rtt) samples — a min-RTT-gated offset
/// plus a linear-regression skew over a sliding window.
///
/// Queuing delay only adds, so only samples within `rttGateMicroseconds`
/// of the window's min RTT join the fit. Regression absorbs ~50 ppm
/// consumer clock skew. A session maps every stream through one model;
/// divergent estimates are an A/V sync error. No clock is read here: time
/// advances through the samples' own coordinates, and the model is usable
...
/// `maxWindowSamples`, the fitted skew is clamped to
/// ±`maxSkewPartsPerMillion` (a frozen host clock fits a slope of exactly
/// 1), and every Double→Int64 conversion saturates. A lying host skews its
/// own frame timing; it never traps the client.
public struct ClientHostClock: Sendable {
    ...
    public mutating func ingest(_ sample: ClockSample) {
    ...
    public func recentSamples(_ limit: Int) -> [ClockSample] {
        Array(window.suffix(max(0, limit)))
    }

    /// The current fit, or nil before the first sample; cached per window
    /// change.
    public mutating func estimate() -> Estimate? {
    ...
    public mutating func map(_ host: HostTimestamp) -> ClientTimestamp? {
        estimate()?.map(host)
    }
}
