// Host audio routing, the client half (HS-18 → CL-13): the wire
// shapes of the "does the host's own speaker keep playing while the
// client streams?" choice, BYTE-MIRRORED from
// Host/Sources/HostWire/AudioRouting.swift — the 0x15/0x16/0x17
// precedent (end-side pins first; promotion into LyteWire is registry
// appends plus file moves, the bytes never change, and both copies
// delete together when the codec-promotion slice lands).
//
// CAPABILITY CARRIAGE — the W7 forward-compat spine, exactly as the
// host pinned it: key 9 (hostAudioRouting, bool) rides the client's
// 0x0F declaration through `Capabilities.unknownEntries` as one
// canonical `09 F5` map entry appended to the frozen v1 encoding.
// ZERO frozen bytes move (capabilities-v1.json never regenerates);
// the capability survives intersection only on byte-equal MUTUAL
// declaration, which is precisely what lets the stream window's
// control strip gate its host-mute button truthfully — against a
// legacy host the key vanishes from the agreed set and the button
// never exists. Absence is "not supported", never an error.
//
...
// the host ACTUALLY applied — sent once at capability agreement (the
// session's starting posture) and after every applied flip; a failed
// flip reports the OLD posture. The strip renders truth, not hope.
// Both layouts are `type ‖ mode u8` (0x01 hostAudible / 0x02
// hostMuted), pinned host-side by AudioRoutingGateTests and mirrored
// byte-exact by this end's gate.
//
...
extension ClientCtrlMessageType {
    /// Client→host audio-routing flip request (HS-18 → CL-13).
    public static let audioRoutingRequest: UInt8 = 0x18
    /// Host→client applied-posture status (HS-18 → CL-13).
    public static let audioRoutingStatus: UInt8 = 0x19
}

/// Capability registry keys pinned client-side awaiting promotion into
/// `CapabilityKey` (see the file comment; the host's twin is
/// `HostCapabilityKey`).
public enum ClientCapabilityKey {
    /// bool — the peer can route host desktop audio to a virtual sink
    /// and mute the host's own speakers for the session (and honors
    /// 0x18 flips). Gated by intersection per W7 rule 3: enabled only
...
public enum HostAudioRoutingMode: UInt8, Hashable, CaseIterable, Sendable {
    /// The HS-14 posture: the host captures its default sink's
    /// monitor; the host's speakers keep playing.
    case hostAudible = 0x01
    /// The virtual-sink posture: "Lyte Audio" becomes the host's
    /// default sink, its monitor feeds the wire, the physical output
    /// is silent.
    case hostMuted = 0x02
}
...
public struct AudioRoutingRequest: Hashable, Sendable {
    ...
    public func encode() -> [UInt8] {
        [ClientCtrlMessageType.audioRoutingRequest, mode.rawValue]
    }
    ...
    public static func decode(
        ...
        AudioRoutingRequest(mode: try decodeRoutingBody(
            payload, type: ClientCtrlMessageType.audioRoutingRequest
        ))
    }
...
public struct AudioRoutingStatus: Hashable, Sendable {
    ...
    public func encode() -> [UInt8] {
        [ClientCtrlMessageType.audioRoutingStatus, mode.rawValue]
    }
    ...
    public static func decode(
        ...
        AudioRoutingStatus(mode: try decodeRoutingBody(
            payload, type: ClientCtrlMessageType.audioRoutingStatus
        ))
    }
...
/// would only drop it loud (`.audioRoutingNotNegotiated`).
