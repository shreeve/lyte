// Input messages (CL-9): the client-side mirror of HS-13's host-pinned
// input-injection wire shapes. The client captures an event, stamps it,
...
// frame").
//
// BYTE MIRROR of Host/Sources/HostWire/InputMessages.swift — the
// IdleFrame precedent (0x15 rode the same mirror-then-promote path at
// CL-8). Type bytes 0x16/0x17 and the lastInputSeq TLV type 0x03 are
// pinned HOST-SIDE FIRST; promotion into LyteWire is a file move plus
// registry appends owned by a Wire/ slice — the bytes never change,
// and both copies delete together when it lands.
//
...
// and typing is the one traffic class where a lost or reordered event
// is corruption, not weather. Latency lives in the priority order, not
// in datagram fire-and-forget. Echo tuples carry TIMESTAMPS, not
// freshness: a late echo still reports the true rx/inject instants, so
// reliable carriage costs the accounting nothing.
//
...
/// Envelope TLV types pinned end-side awaiting promotion into
/// `WireExtension.ReservedType` (see the file comment).
public enum ClientWireExtensionType {
...
public struct InputEvent: Hashable, Sendable {
    ...
    public func encode() -> [UInt8] {
        ...
        out.append(ClientCtrlMessageType.inputEvent)
        appendLE(&out, seq)
    ...
    public static func decode(_ payload: ArraySlice<UInt8>) throws -> InputEvent {
        ...
        guard payload[base] == ClientCtrlMessageType.inputEvent else {
...
public struct InputEcho: Hashable, Sendable {
    ...
    public func encode() -> [UInt8] {
        ...
        out.append(ClientCtrlMessageType.inputEcho)
    ...
    public static func decode(_ payload: ArraySlice<UInt8>) throws -> InputEcho {
        ...
        let base = payload.startIndex
        guard payload[base] == ClientCtrlMessageType.inputEcho else {
            throw InputMessageError.unexpectedType(payload[base])
        }
        ...
        guard payload.count == 2 + count * tupleByteCount else {
            throw InputMessageError.bodyLengthMismatch(
                kind: ClientCtrlMessageType.inputEcho,
...
public enum LastInputSeqTlv {
    ...
    public static func wireExtension(seq: UInt32) -> WireExtension {
        ...
        return try! WireExtension(
            type: ClientWireExtensionType.lastInputSeq, value: value
        )
    }
    ...
    public static func decode(extensions: [WireExtension]) throws -> UInt32? {
        let matches = extensions.filter {
            $0.type == ClientWireExtensionType.lastInputSeq
        }
