// Input messages (HS-13): the wire shapes of the input-injection path.
// The client captures an event, stamps it, sequences it, and sends it on
// the sealed reliable CTRL stream; the host injects it into the desktop
// session and answers with echo tuples so the client can close the
// per-keystroke input-to-photon loop (master plan HS-13/CL-9 rows;
// overview: "input events (client-timestamped, sequenced) ride CTRL; the
// host injects, echoes (seq, rx ts, inject ts), and stamps lastInputSeq
// into the next frame").
//
...
//
// PROMOTE-TO-LyteWire FLAG (with CL-9, the client's input slice): type
// bytes 0x16/0x17 and the lastInputSeq TLV type 0x03 are pinned
// HOST-SIDE FIRST — the HS-7/HS-12/HS-11 precedent (0x03–0x06, 0x10,
// 0x15 all landed this way) — because Wire/ is another worker's
// territory tonight. 0x16/0x17 are the first bytes past HS-11's 0x15;
// TLV 0x03 is the first past W0's reserved 0x00–0x02. Promotion is a
// file move plus registry appends; the bytes never change.
//
...
extension HostCtrlMessageType {
    /// Client→host input event (HS-13 → CL-9).
    public static let inputEvent: UInt8 = 0x16
    /// Host→client input echo tuples (HS-13 → CL-9).
    public static let inputEcho: UInt8 = 0x17
}

/// Envelope TLV types pinned host-side awaiting promotion into
/// `WireExtension.ReservedType` (see the file comment).
public enum HostWireExtensionType {
    /// u32 LE: the seq of the last input event injected before this
    /// frame's capture — stamped on every shard of every video frame
    /// once any input has been injected (per-shard like the conn-id
    /// TLV, because shards are independently lossy and the client must
    /// recover the association from any one of them).
...
public struct InputEvent: Hashable, Sendable {
    ...
    public func encode() -> [UInt8] {
        ...
        out.append(HostCtrlMessageType.inputEvent)
        appendLE(&out, seq)
    ...
    public static func decode(_ payload: ArraySlice<UInt8>) throws -> InputEvent {
        ...
        guard payload[base] == HostCtrlMessageType.inputEvent else {
...
public struct InputEcho: Hashable, Sendable {
    ...
    /// a host bug, not hostile input (decode rejects the same shapes).
    ...
    public func encode() -> [UInt8] {
        ...
        out.append(HostCtrlMessageType.inputEcho)
    ...
    public static func decode(_ payload: ArraySlice<UInt8>) throws -> InputEcho {
        ...
        let base = payload.startIndex
        guard payload[base] == HostCtrlMessageType.inputEcho else {
            throw InputMessageError.unexpectedType(payload[base])
        }
        ...
        guard payload.count == 2 + count * tupleByteCount else {
            throw InputMessageError.bodyLengthMismatch(
                kind: HostCtrlMessageType.inputEcho,
...
public enum LastInputSeqTlv {
    ...
    public static func wireExtension(seq: UInt32) -> WireExtension {
        ...
        return try! WireExtension(
            type: HostWireExtensionType.lastInputSeq, value: value
        )
    }
    ...
    public static func decode(extensions: [WireExtension]) throws -> UInt32? {
        let matches = extensions.filter {
            $0.type == HostWireExtensionType.lastInputSeq
        }
