...
// `tlv-reserved-types` vector). W0 reserved the number only; this is the
// VALUE codec, landed host-side with HS-12.
//
// ── PROMOTE TO LyteWire ──────────────────────────────────────────────────
// The client needs the byte-identical codec (CL side tags its datagrams
// with the same TLV and must recognize the host's, or migration cannot be
// attributed in either direction). When the CL migration slice lands, lift
// this file into Wire/Sources/LyteWire verbatim and delete it here — the
// API deliberately touches nothing but LyteWire types so the move is a
// `git mv`. Flagged in the HS-12 commit body.
// ─────────────────────────────────────────────────────────────────────────
//
