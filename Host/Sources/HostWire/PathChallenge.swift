...
//
// CTRL type numbers 0x03/0x04 are pinned HERE, host-side, because W4a's
// registry stopped at 0x02 and Wire/ is not this slice's territory. They
// are the next free values and follow the registry's rules (0x00 stays
// invalid). Promote to LyteWire's CtrlMessageType together with the
// ConnectionId codec when the client's migration slice lands — the client
// must echo these exact bytes.
//
...
public enum HostCtrlMessageType {
    /// Host→client path-validation challenge (QUIC §9's PATH_CHALLENGE).
    public static let pathChallenge: UInt8 = 0x03
    /// Client→host echo of a challenge token (PATH_RESPONSE).
...
public struct PathChallenge: Hashable, Sendable {
    ...
    public func encode() -> [UInt8] {
        encodePathMessage(type: HostCtrlMessageType.pathChallenge,
                          token: token)
    }

    public static func decode(_ payload: [UInt8]) throws -> PathChallenge {
        PathChallenge(token: try decodePathMessage(
            payload, expecting: HostCtrlMessageType.pathChallenge
        ))
    }
}
...
public struct PathResponse: Hashable, Sendable {
    ...
    public func encode() -> [UInt8] {
        encodePathMessage(type: HostCtrlMessageType.pathResponse,
                          token: token)
    }

    public static func decode(_ payload: [UInt8]) throws -> PathResponse {
        PathResponse(token: try decodePathMessage(
            payload, expecting: HostCtrlMessageType.pathResponse
        ))
    }

    /// The well-formed echo of a challenge — the client-side rule, here
    /// for the loopback tests and, later, the promoted shared codec.
...
private func encodePathMessage(type: UInt8, token: UInt64) -> [UInt8] {
    ...
    for shift in stride(from: 0, to: 64, by: 8) {
        out.append(UInt8(truncatingIfNeeded: token >> shift))
    }
    return out
}
