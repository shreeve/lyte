// AudioFramer (HS-15): 5 ms Opus packets from the HS-14 capture path →
// audio-channel datagrams under the Lyte-UDP envelope, with the
// resiliency pillar's 4+2 Reed-Solomon interleave expressed through the
// SAME frozen Wire machinery video uses (FecField, FecGeometry,
// FecEncoder — the nanors codebook). Sans-IO in the HostCore style: no
// sockets, no threads, no clock — capture timestamps are injected, the
// caller owns pacing and syscalls. This file COMPOSES frozen wire
// formats for a new channel; it changes none of them.
//
// HOST-PINNED wire layout (the 0x15/0x16 precedent — the master plan's
// W8 row promised "Opus framing + 4+2 RS in fec field" in Wire/, but
// that slot went to the retry cookie and Wire/ is another territory's
// tonight; the client mirrors this byte-for-byte at CL-11 and both
// copies promote into Wire/ together):
//
...
//
// Sealing is the caller's (Session's) job — same header-bytes-as-AAD
// discipline as every other datagram; this framer emits (envelope,
// plaintext payload) pairs so the layout is pinnable without crypto.
