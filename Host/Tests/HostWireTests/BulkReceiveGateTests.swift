...
final class BulkReceiveGateTests: XCTestCase {
    ...
    // MARK: Leg 5 — the streaming digest never grades its own homework

    func testGateSha256StreamMatchesReferenceVectorsAndChunking() {
        // FIPS 180-2 vectors, hand-transcribed.
        XCTAssertEqual(
            Sha256Stream.digest(Array("abc".utf8)),
        ...
        // Streaming across hostile split boundaries == the TestKit
        // reference digest (which is its own independent code path).
        ...
        print("F-3 gate (digest): FIPS vectors pinned; streaming splits "
            + "match the TestKit reference on 200,001 B")
    }
